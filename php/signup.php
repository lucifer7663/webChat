...
session_start();
    ...
    if(!empty($fname) && !empty($lname) && !empty($email) && !empty($password)){
        // lets check if user is valid or not
        if(filter_var($email, FILTER_VALIDATE_EMAIL)){
            //lets check email is already registered or not
            $sql = mysqli_query($conn, "SELECT email FROM users WHERE email = '{$email}'");
            if(mysqli_num_rows($sql) > 0){
                echo "$email - is already exist";
            }else{
                //lets check user upload fields
                if(isset($_FILES['image'])){ //if file is uploaded
                    $img_name = $_FILES['image']['name']; // getting user uploded img name
                    $img_type = $_FILES['image']['type']; //getting user uploded img type
                    $tmp_name = $_FILES['image']['tmp_name']; //this is for saving/moving file in our folder
